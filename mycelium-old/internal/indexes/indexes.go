...
func SetupIndexes(ctx context.Context, mgr manager.Manager) error {
	...
	if err := mgr.GetFieldIndexer().IndexField(ctx, &v1alpha1.MyceliumTool{},
	...
	// Agent → Tool refs
	if err := mgr.GetFieldIndexer().IndexField(ctx, &v1alpha1.MyceliumAgent{},
	...
	if err := mgr.GetFieldIndexer().IndexField(ctx, &v1alpha1.MyceliumCredentialProvider{},
		IndexCredentialProviderSecrets,
		func(obj client.Object) []string {
			cp := obj.(*v1alpha1.MyceliumCredentialProvider)
			...
			case v1alpha1.CredentialProviderTypeAPIKey:
				return []string{cp.Spec.APIKey.SecretRef.Name}
			}
			return nil
		},
	); err != nil {
