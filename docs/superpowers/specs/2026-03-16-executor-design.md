...
**Date:** 2026-03-16
**Status:** Draft (v2 — pasta-based architecture)
...
- **STARTING**: VM booting. `/run` returns 503. `/healthz` returns 503.
- **RUNNING**: Agent handling request, SSE streaming. `/run` returns 503.
- **TEARDOWN**: VM shutting down, lease being released. `/run` returns 503.

## Pool Operator Integration

### Executor contract

1. **Readiness probe**: `/healthz` returns 200 only when IDLE.
2. **Lease renewal**: On `/run`, start a goroutine calling gRPC `Renew` every `leaseTTL/3`.
3. **Release on completion**: After teardown, call gRPC `Release`. Best effort, 3 retries (1s/2s/4s backoff).
4. **Reuse safety**: Tear down VM and working directory before calling `Release`. State machine prevents concurrent `/run`.
...
| `READY_TIMEOUT` | `10s` | Max time for agent to start |
| `EXEC_TIMEOUT` | `5m` | Max execution time |
...
```
 ...
 4. Waypoint calls gRPC Claim to pool operator → gets {pod_ip, claim_id}
 ...
 6. Executor:
    ...
    c. Starts lease renewal goroutine (gRPC Renew every 10s)
    d. Boots Firecracker VM in pasta's dedicated netns
    e. Guest init: mounts rootfs, configures network (via vsock Init),
       execs into agent binary
    f. Executor polls localhost:8080/healthz until agent is ready
 7. Executor:
    a. Transitions STARTING → RUNNING
    b. POST http://localhost:8080/run with client payload
    c. Streams SSE response from agent back to waypoint HTTP response
 8. During execution, agent makes outbound calls (LLM, tools, A2A):
    Agent → VM eth0 → TAP → pasta L2→L4 → socket in root netns
    → ztunnel OUTPUT chain capture → HBONE → tenant waypoint
    (credential injection, prompt guard) → external API
 9. SSE stream ends
10. Executor:
    a. Transitions RUNNING → TEARDOWN
    b. Kills Firecracker process, VM shuts down
    c. Cleans working directory
    d. Stops lease renewal goroutine
    e. Calls gRPC Release to pool operator
