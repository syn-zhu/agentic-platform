...
```
                        ┌─────────────────────────────────────┐
                        │          AgentGateway Proxy          │
                        │    (L7: MCP, A2A, LLM routing)      │
                        │         gateway node group           │
                        └──────┬──────────┬──────────┬────────┘
                               │          │          │
                    ┌──────────┘    ┌─────┘    ┌─────┘
                    ▼               ▼          ▼
             /a2a/{ns}/{name} /llm/{ns}/{model} /mcp/{ns}/{server}
                    │               │          │
                    ▼               ▼          ▼
        ┌───────────────┐  ┌──────────────┐  ┌──────────────┐
        │ Tenant Waypoint│  │  LLM Backend │  │  MCP Server  │
        │  (per-tenant   │  │  (Anthropic,  │  │  (tools,     │
        │   L7 proxy)    │  │   OpenAI...) │  │   filesys...) │
        └───────┬────────┘  └──────────────┘  └──────────────┘
                │
    ┌───────────┼───────────┐
    ▼           ▼           ▼
┌────────┐ ┌────────┐ ┌────────┐
│Agent A │ │Agent B │ │Sandbox │    ← agent node group
└────────┘ └────────┘ └────────┘
```
...
Agent interactions are traced end-to-end:

```
Agent Pod → (gRPC OTLP) → OTEL Collector → (HTTP OTLP) → Langfuse
AgentGateway → (HTTP OTLP) → Langfuse
```

Langfuse captures prompt/completion pairs, token usage, latency, and cost. Traces follow W3C `traceparent` headers across agent-to-agent calls (Kyverno injects `DISABLE_AIOHTTP_TRANSPORT=True` to ensure `httpx` propagates context correctly).
